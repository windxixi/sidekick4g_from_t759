...
#if defined (CONFIG_S5PC110_HAWK_BOARD) /* nat */
#define DISPLAY_SDO      S5PV210_MP04(4)
#endif
...
/*ldo 13*/
	if (IS_ERR_OR_NULL(vga_core_regulator)) {
		vga_core_regulator = regulator_get(NULL, "vga_vddio");
		if (IS_ERR_OR_NULL(vga_core_regulator)) {
...
{	
	...
	// Mclk enable
	s3c_gpio_cfgpin(GPIO_CAM_MCLK, (0x2 << 12));
...
{	
	...
	/* CAM_VGA_EN - GPJ1(2) */
	err = gpio_request(GPIO_CAM_VGA_nSTBY, "GPJ12");
	if(err) {
	...
	/* CAM_VGA_nRST - GPJ1(4) */
	err = gpio_request(GPIO_CAM_VGA_nRST, "GPJ14");
	if(err) {
...
//&s3c_device_i2c8, 
	...
	&s3c_device_i2c14, /* magnetic sensor */
